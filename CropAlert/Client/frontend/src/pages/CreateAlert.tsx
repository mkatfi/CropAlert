...
const CreateAlert: React.FC = () => {
  ...
  const editingZone = location.state?.editingZone as ZoneData | undefined;
  ...
  const handleUseZoneLocation = async (zoneId: number) => {
    try {
      ...
      if (zone) {
        ...
        toast.success(`Location set to Zone ${zoneId} coordinates`);
      } else {
        toast.error('Zone not found');
      }
    } catch (error) {
      toast.error('Failed to get zone location');
    }
  };
    ...
    try {
      if (editingZone) {
        ...
        const zoneData = {
          ...
          userId: user?.id || farmers[0]?.id || 1, // Default to current user or first farmer
        };
        ...
        // If zone created successfully, you might want to also update it with alert details
        if (createdZone) {
          await zoneService.updateZone(createdZone.id, {
            title: formData.title,
            description: `${formData.description} (Crop: ${formData.cropType}, Severity: ${formData.severity})`,
            status: formData.severity === 'critical' ? 'active' : 'pending'
          });
        }
        
        toast.success('Alert/Zone created successfully!');
        fetchZonesAndFarmers(); // Refresh the zones list
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-blue-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Create New Alert</h1>
          <p className="text-gray-600">
            Share important crop information with farmers in your area
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    ...
                    {zones.length > 0 && (
                      <div className="border-t pt-2">
                        <p className="text-sm text-gray-600 mb-2">Or use existing zone location:</p>
                        <select
                          onChange={(e) => {
                            const zoneId = parseInt(e.target.value);
                            if (zoneId) {
                              handleUseZoneLocation(zoneId);
                            }
                          }}
                        ...
                        >
                          ...
                          {zones.map(zone => (
                            <option key={zone.id} value={zone.id}>
                              {zone.title || `Zone ${zone.id}`} - ({zone.latitude.toFixed(4)}, {zone.longitude.toFixed(4)})
                            </option>
                          ))}
                        </select>
                      </div>
          ...
          >
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Alert Location Preview</h3>
...
// Zone Card Component (copied from NewDashboard with modifications for CreateAlert context)
